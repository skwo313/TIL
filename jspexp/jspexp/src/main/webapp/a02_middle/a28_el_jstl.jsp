...
<body>
	...
	<table>
		...
		<tr><th>사는 곳</th><td>${p02.loc }</td></tr>
	</table>
	
	<%
		request.setAttribute("prod01",new Cart("사과",3000,2));
	%>
	<c:set var="prod02" value="${prod01 }" scope="session"/>
	<c:set target="${prod02 }" property="pname" value="바나나"/>
	<c:set target="${prod02 }" property="price" value="4000"/>
	<c:set target="${prod02 }" property="cnt" value="5"/>
	<table>
		<tr><th>물건명</th><td>${prod02.pname }</td></tr>
