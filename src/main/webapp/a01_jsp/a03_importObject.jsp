...
<body>
	...
	2. 처리 방법
		...
		3) 해당 위치의 클래스파일의 패키지명과 클래스명을 복사
		4) jsp 상단의 page지시자(<%@ page)의 속성에 속성을 import="패키지명.클래스"으로 선언
		5) <%%> 코드 사이에 객체를 생성 및 생성자를 통해서 데이터 할당
		6) 테이블 구조를 만들고 해당 구조안에 참조변수.getXXX() 메서드를 통해서 필드값 간접적으로 호출
		7) 에러 내용/코드 확인 및 실행 처리
	# 예제
	 --%>
	 <%
