...
3. 계층형 sql의 데이터 구조
	emp테이블의 mgr과 empno는 계층형관계에 있다. 즉, 하나의 사원 정보 row는
	사원번호(empno)와 관리자번호(mgr)가 있는데, 이 mgr은 사원번호를 다시 가지고 있다.

4. 계층형 sql의 주요 기능
	1) sql처리시, 상위에 있는 데이터를 하위에 바로 나올 수 있게 데이터를 처리할 수 있다.
	2) 가장 밑에 node부터 나오고, 형제 레밸로 나오는 등, 계층형 데이터를 우선 순위로 나오게 할 수 있다.
	ex)		7839
	7782  	7698  		7566
	7934  7844 7900..	7902
	답변형 게시판에서 이런 계층sql을 이용해서 처리하는 경우가 많다.
				게시물1		게시물2		게시물3
			답글1	  답글2	   답글1  답글2	
	...
	3) level을 통해서 계층의 레밸을 나타낼 수 있다.
	4) 계층의 root 위치를 지정해서 해당 계층에서부터 처리도 가능하게 한다.
	
5. 계층형 sql의 주요 키워드
	1) select에서 쓰이는 키워드 : 
		level : 각 컬럼에 해당하는 데이터의 계층 레벨을 표기한다.
	2) start width 컬럼 = 조건 : 최상위 root 레벨을 지칭하는 조건이다
...
-- 상/하위 컬럼에 대한 내용을 조건으로 순위에 맞개 지정하여야 한다. 하위컬럼 = 상위컬럼
/*
		가족계층형 테이블을 구성하여,
	start with connect by prior 구문을 이용하여,
	출력처리하세요
		...
		아버지(김철수)			작은아버지(김철민)
나(김신화) 누나(김영애)		사촌(김민희)
--1. 테이블을 상하위 기준이 되는 컬럼을 설정해서 필요한 컬럼을 추가. :no, refno
...
SELECT LEVEL, f.*
FROM family f
START WITH refno=0 -- 최상위 root 노드를 지정.
